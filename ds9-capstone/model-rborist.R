...
library(Rborist)                # strong parallel computing
...
gridSearch <- expand.grid(predFixed = seq(2, maxGrid, by = 3),
                          minNode   = seq(500, 1500, 500)
)

set.seed(1, sample.kind = "Rounding")
rf_arborist <- df_train %>%
  train(rating ~ .,
        method = "Rborist",
        data = .,
        nTree=100,
...
ggplot(rf_arborist, highlight = TRUE)
rf_arborist$bestTune
rf_arborist$finalModel

# predict the outcome
print("predict the outcome")
predicted <- predict(rf_arborist, df_test)
...
# save model
print("save model")
save(rf_arborist, file="./mdl/rf_arborist.RData")

# clean memory
rm(rf_arborist, df, df_train, df_test)
