...
# setup layers
D <- ncol(X_train)
inputLayer <- layer_input(shape = c(D))

modelLayers <- inputLayer %>%
  layer_dense(units = D, activation = 'relu') %>%
  layer_dense(units = D, activation = 'relu') %>%
  # layer_dropout(0.2) %>%
  layer_dense(units = 10, activation = 'softmax')   # 10-classes classification layer

model <- keras_model(inputs = inputLayer, outputs = modelLayers)
